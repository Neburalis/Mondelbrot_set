// mandelbrot with simple batch processing (We hope that the compiler will be able to optimize)
...
static void handle_input(double *origin_x, double *origin_y, double *scale, double step)
{
    if (!disable_window) {
        if (IsKeyDown(KEY_RIGHT)) *origin_x += step;
        else if (IsKeyDown(KEY_LEFT))  *origin_x -= step;
        if (IsKeyDown(KEY_DOWN))  *origin_y += step;
        else if (IsKeyDown(KEY_UP))    *origin_y -= step;
...
static Color compute_pixel_color(int n, int MaxN, double X, double Y)
{
    ...
    double r2 = X*X + Y*Y;
    double log2X = (r2 > 0.0f) ?
        (double)n + 1.0f - log2f(log2f(r2) * 0.5f) : (double)n;
    double t = log2X / (double)MaxN;
    double h = t * 6.28318f * 3.0f;
    double r = 0.5f + 0.5f * cosf(h);
    double g = 0.5f + 0.5f * cosf(h + 2.094f);
    double b = 0.5f + 0.5f * cosf(h + 4.189f);
    double brightness = 1.0f - expf(-log2X * 0.15f);
    return (Color){
...
static void compute_mandelbrot(Color *pixels, int ScreenWidth, int ScreenHeight,
                                int MaxN, double origin_x, double origin_y, double scale)
{
    #define wide 8
    const double rMAX = 10 * 10;
    for (int iy = 0; iy < ScreenHeight; ++iy) {
        double P0x[wide] = {origin_x, origin_x + scale, origin_x + 2 * scale, origin_x + 3 * scale, origin_x + 4 * scale, origin_x + 5 * scale, origin_x + 6 * scale, origin_x + 7 * scale},
        ...
        for (int ix = 0; ix < ScreenWidth; ix += wide) {
            double X[wide], Y[wide];
            for (int i = 0; i < wide; ++i) X[i] = P0x[i];
            for (int i = 0; i < wide; ++i) Y[i] = P0y[i];
            double EscX[wide] = {0}, EscY[wide] = {0};
            int n, alive = wide, N[wide] = {0}, step[wide] = {1, 1, 1, 1, 1, 1, 1, 1};
            for (n = 0; n < MaxN; ++n) {
                double X2[wide]; for (int i = 0; i < wide; ++i) X2[i] = X[i] * X[i];
                double Y2[wide]; for (int i = 0; i < wide; ++i) Y2[i] = Y[i] * Y[i];
                double XY[wide]; for (int i = 0; i < wide; ++i) XY[i] = X[i] * Y[i];
...
int main(void)
{
    ...
    double origin_x = -2.4f, origin_y = -0.55f, scale = 1.8f / 800.0f;
    ...
    {
        handle_input(&origin_x, &origin_y, &scale, scale * 20);

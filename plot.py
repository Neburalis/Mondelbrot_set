...
stdev  = statistics.stdev(data_ms) if n > 1 else 0.0
...
fig.suptitle(f"Frame times — {pathlib.Path(path).name}  "
             f"({n} frames, mean {mean:.2f} ms / {1000/mean:.1f} fps)")

ax = axes[0]
ax.plot(frames, data_ms, linewidth=0.5, color="steelblue", alpha=0.8)
ax.axhline(mean,   color="orange", linewidth=1.2, label=f"mean {mean:.2f} ms")
...
ax.axvline(mean,   color="orange", linewidth=1.5, label=f"mean {mean:.2f} ms")

                                ...
                                int MaxN, float origin_x, float origin_y, float scale)
{
    ...
    for (int iy = 0; iy < ScreenHeight; ++iy) {
        float P0x[4] = {origin_x, origin_x + scale, origin_x + 2 * scale, origin_x + 3 * scale},
              P0y[4] = {origin_y + iy * scale, origin_y + iy * scale, origin_y + iy * scale, origin_y + iy * scale};
        for (int ix = 0; ix < ScreenWidth; ix += 4) {
            float X[4], Y[4];
            for (int i = 0; i < 4; ++i) X[i] = P0x[i];
            for (int i = 0; i < 4; ++i) Y[i] = P0y[i];
            int n, alive = 4, N[4] = {0}, step[4] = {1, 1, 1, 1};
            for (n = 0; n < MaxN; ++n) {
                ...
                if (!alive) break;
                // if (X2 + Y2 >= rMAX) break;
                for (int i = 0; i < 4; ++i) X[i] = X2[i] - Y2[i] + P0x[i];
                for (int i = 0; i < 4; ++i) Y[i] = XY[i] + XY[i] + P0y[i];
                // X = X2 - Y2 + P0x;
                // Y = XY + XY + P0y;
                for (int i = 0; i < 4; ++i) N[i] += step[i];
            }
            for (int i = 0; i < 4; ++i)
                pixels[ix + i + ScreenWidth * iy] = compute_pixel_color(N[i], MaxN, X[i], Y[i]);
            for (int i = 0; i < 4; ++i) P0x[i] += scale * 4;
        }
    }
}
